...
class EEGFineTuningDataset:
    ...
    def __init__(
        ...
        # Adding a default chat template if missing (Jinja2 templating; used in Hugging Face tokenizer templating)
        if not hasattr(self.tokenizer, "chat_template") or self.tokenizer.chat_template is None:
            self.tokenizer.chat_template = (
                "{% for message in messages %}"
                "{% if message['role'] == 'system' %}<s>System: {{ message['content'] }}</s>\n{% endif %}"
                "{% if message['role'] == 'user' %}<s>User: {{ message['content'] }}</s>\n{% endif %}"
                "{% if message['role'] == 'assistant' %}<s>Assistant: {{ message['content'] }}</s>\n{% endif %}"
                "{% endfor %}"
            )
